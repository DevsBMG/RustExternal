...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImU32 outer_border_color = IM_COL32(0, 0, 0, 255);
    const ImVec2 outer_border_min = ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y - 1);
    const ImVec2 outer_border_max = ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1);
    window->DrawList->AddRect(outer_border_min, outer_border_max, outer_border_color, 0.0f, 0, 1.0f);

    ImU32 colors[] = {
        ImGui::ColorConvertFloat4ToU32(ImColor(23, 22, 25, 255)),
        ImGui::ColorConvertFloat4ToU32(ImColor(25, 24, 26, 255)),
        ImGui::ColorConvertFloat4ToU32(ImColor(33, 33, 37, 255)),
        ImGui::ColorConvertFloat4ToU32(ImColor(37, 35, 40, 255))
    };

    window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, colors[0], colors[1], colors[2], colors[3]);

    const ImU32 last_rect_color = IM_COL32(50, 50, 50, 255);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, last_rect_color);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ImVec2 outer_border_min = ImVec2(grab_bb.Min.x - 1, grab_bb.Min.y - 1);
        ImVec2 outer_border_max = ImVec2(grab_bb.Max.x + 1, grab_bb.Max.y + 1);
        window->DrawList->AddRect(outer_border_min, outer_border_max, IM_COL32(0, 0, 0, 255), 0.0f, 0, 1.0f);
        window->DrawList->AddRectFilledMultiColor(grab_bb.Min, grab_bb.Max, colors[0], colors[1], colors[2], colors[3]);
        window->DrawList->AddRect(grab_bb.Min, grab_bb.Max, last_rect_color);
    }
